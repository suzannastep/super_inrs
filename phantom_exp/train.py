...
def train_validate_split(seed,y,split=0.8):
...
def main():
    ...
    with wandb.init(project="middle_linear", config=settings, group="super INRs", save_code=True, job_type = "train") as run:
        ...
        inr, x, metrics = train_network(settings,device)
...
def train_network(settings,device):
    #load data
    ...
    train_idx, validate_idx = train_validate_split(settings['datasplitseed'],y,split=settings['datasplit'])
    y_train = y[train_idx]
